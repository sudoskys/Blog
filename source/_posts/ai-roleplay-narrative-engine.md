...
当 $K(S) > W$ 时，状态漂移不可避免——某些世界事实必须被截断或有损压缩。更严格地说，设压缩映射为 $\pi: S \to \hat{S}$，其失真度 $d(S, \hat{S}) = |S \triangle \hat{S}|$（对称差的势，即被错误添加或遗漏的事实数量），则：

$$\mathbb{E}[d(S,\hat{S})] \geq K(S) - W \quad \text{（信息论下界）} \tag{1}$$

这意味着状态漂移不是实现缺陷，而是**信息论必然**。任何基于有限窗口的方案——无论是摘要压缩、世界书注入还是 RAG 检索——都无法将 $d$ 压至零。这一观察构成了本文的出发点：既然完美保真不可能，我们需要一种框架来判断**哪些事实最重要**、**什么时候遗忘是可接受的**、以及**系统应如何自动恢复一致性**。
...
**定理 2**（契诃夫完备性的不可判定性）。设 $\mathcal{N}$ 为叙事图的无穷演化序列 $(G_0, G_1, G_2, \ldots)$，判定「序列 $\mathcal{N}$ 是否满足定律 IV」是 $\Sigma_1^0$-完备的。

$\Sigma_1^0$ 是算术层级的第一层，包含所有形如「$\exists n: P(n)$」的可递归枚举谓词。$\Sigma_1^0$-完备意味着该问题与停机问题等价：不存在通用算法能判定任意叙事序列是否满足契诃夫守恒。

*证明*。我们构造从停机问题到契诃夫完备性判定的归约。给定图灵机 $M$ 和输入 $x$，构造叙事图序列 $\mathcal{N}_M$：

1. 在 $G_0$ 中植入一个特殊的 Thread $t_M$，$\text{status}(t_M) = \texttt{planted}$。
2. 定义演化规则：在步骤 $n$ 中，模拟 $M(x)$ 的第 $n$ 步。
3. 当且仅当 $M(x)$ 在步骤 $n$ 停机时，令 $\text{status}(t_M) = \texttt{paid}$。

则 $\mathcal{N}_M$ 满足定律 IV 当且仅当 $M(x)$ 停机。因此，判定定律 IV 至少与停机问题一样难。反方向：定律 IV 可以表示为 $\exists n: \text{status}(t) \in \{\texttt{paid}, \texttt{abandoned}\}$，这是 $\Sigma_1^0$ 形式的谓词。故问题是 $\Sigma_1^0$-完备的。$\square$
...
**为什么选择超线性增长**？$\beta > 1$ 意味着压力随时间加速增长——第 10 场景时的压力远大于第 5 场景时的两倍。这不是任意选择，而是有认知心理学依据：Stevens 幂律[^1]指出，人对刺激强度的主观感知 $\psi$ 与物理强度 $I$ 之间满足 $\psi = k \cdot I^\beta$，其中 $\beta > 1$ 对应加速增长的刺激（如疼痛、电击——以及悬念的折磨）。观众对未兑现承诺的不耐烦是加速增长的，而非线性的——「等了 3 集」和「等了 10 集」的差距远大于 3 倍。
...
其中 $\mathcal{K}(\sigma) = \{c \mid (c, \texttt{KNOWS}, \sigma) \in E\}$（知晓者集合），$\mathcal{S}_?(\sigma) = \{c \mid (c, \texttt{SUSPECTS}, \sigma) \in E\}$（怀疑者集合）。

**直觉**。一个秘密的揭示压力取决于两个因素：(1) 它的内在张力 $\tau(\sigma)$——一个「Bob 喜欢吃苹果」的秘密和一个「Bob 杀了 Alice 的父亲」的秘密有天壤之别；(2) 社会放大效应——知道秘密的人越多（泄露风险越大）、怀疑的人越多（接近真相的人越多），压力越大。
...
当所有角色都知道（$H = 0$，无信息不对称）或所有人都不知道（$H = 0$，无信息流通可能），张力最低。当知晓者与无知者共存时，条件熵达到最大——这正是秘密最具叙事张力的状态。$P_\sigma$ 可视为此条件熵的一阶线性近似。
...
$\omega: \mathcal{L} \to \mathbb{R}_{>0}$ 为力的传导权重函数——不同类型的力传导压力的效率不同。例如，TRUSTS 边可能比 NEAR 边更有效地传导揭示压力（你更可能通过信任的人知道秘密，而不是通过地理邻近）。对应的 Laplacian $L_\omega = D_\omega - A_\omega$，特征值谱 $0 = \lambda_1 \leq \lambda_2 \leq \cdots \leq \lambda_N$。
...
**引理 1**（自然熵增）。在无任何植入或解决操作的「空转」场景中（即 $\Delta H_{\text{plant}} = \Delta H_{\text{resolve}} = 0$），熵仍然增长：

$$h_{\text{idle}}(n) = \sum_{t \in \mathcal{T}_{\text{open}}} \left[\text{age}(t,n)^\beta - \text{age}(t,n-1)^\beta\right] \cdot w(t) > 0$$

*证明*。因 $\beta > 1$ 且 $\text{age} \geq 1$，函数 $f(x) = x^\beta$ 严格递增且严格凸（$f'' > 0$）。对任意 $x \geq 1$，$f(x+1) - f(x) > 0$ 且差分递增。每个开放 Thread 的贡献为正，至少有一个开放 Thread（否则 $\mathcal{T}_{\text{open}} = \emptyset$，$h_{\text{idle}} = 0$——但此时无伏笔，熵本身已为零）。$\square$
...
**Remark 9**。定理 4 的意义在于：三幕结构不是一个任意的审美偏好，而是叙事熵动力学在特定植入/解决策略下的**数学必然**。给定超线性的兑现压力（$\beta > 1$）和「先植入后解决」的策略，熵曲线必然呈现先升后降的形态。三幕结构之所以「有效」，不是因为编剧们碰巧达成了共识，而是因为这种熵波形对应了观众心理上最优的张力体验（参见 Conjecture 4）。
...
**动机**。认知心理学中的 Yerkes-Dodson 定律[^4]描述了刺激强度与表现/参与度之间的倒 U 形关系：太低的刺激导致无聊（under-arousal），太高的刺激导致焦虑（over-arousal），最优区间对应心流状态（Csikszentmihalyi[^5]）。

这里的熵产率 $h(n)$ 扮演「叙事刺激强度」的角色。Sigmoid 函数 $\Phi$ 的选择反映了参与度在两端饱和的特性——人不会比「完全不感兴趣」更无聊，也不会比「完全迷失」更困惑。

**推论**。最优叙事策略是维持 $h(n)$ 在 $[h_{\min}, h_{\max}]$ 内振荡。张力压力 $P_\theta$（定义 5-V）正是为此设计的自稳定器。$h_{\min}$ 和 $h_{\max}$ 的具体数值需要通过实验确定，可能因叙事类型（冒险、悬疑、日常）和受众偏好而异。
...
**证明思路**。当 $H > H^* + \varepsilon$ 时，$P_\theta < 0$ 产生释放压力，$h(n)$ 中出现负项使 $H$ 下降，故 $\Delta V < 0$。对称地，当 $H < H^* - \varepsilon$ 时，$P_\theta > 0$ 产生升温压力使 $H$ 上升。严格证明需要对压力函数施加 Lipschitz 连续性条件以控制单步变化量，确保 $P_\theta$ 的调控不会因其他压力的剧烈变化而被淹没。具体地，需要 $|\delta_\pm| > \sup_n |h_{\text{idle}}(n) + \Delta H_{\text{plant}}(n) - \Delta H_{\text{resolve}}(n)|$ 在目标偏差超过 $\varepsilon$ 的区域内成立——这对 $\delta_\pm$ 的值给出了一个下界。
...
**压力计算**（设 $\beta = 1.5$，$w_{\text{major}} = 2$，$\alpha_k = 0.5$，$\alpha_s = 0.3$）：
- $P_\tau(t_1, 1) = 1^{1.5} \cdot 2 = 2.0$
- $P_\sigma(\sigma_1, G_1) = 9 \cdot (1 + 0.5 \cdot 1) = 13.5$（1 个知晓者，0 个怀疑者）
- $P_\gamma = 0$（目前无冲突 Goal 对）
- $P_\rho = 0$

$$H(G_1) = 2.0 + 13.5 = 15.5, \quad h(1) = +15.5$$

从叙事状态 $H = 0$ 跃升至 $H = 15.5$——这是一个典型的 Act I 「Catalyst」节拍：一个重大的戏剧性问题被引入。
...
| 4 | Alice SUSPECTS $\sigma_1$ | $4^{1.5} \cdot 2 = 16.0$ | $9(1+0.5+0.3) = 16.2$ | 32.2 | +8.3 |
| 5 | （空转） | $5^{1.5} \cdot 2 \approx 22.4$ | 16.2 | 38.6 | +6.4 |

**观察**：

1. $n = 2, 3$ 的空转场景中 $h > 0$（引理 1），且 $h$ 递增（$3.7 \to 4.7$）——这正是自然熵增的超线性效应。
2. $n = 4$ 时，Alice 开始怀疑秘密。这增加了一个 SUSPECTS 边，使 $P_\sigma$ 从 13.5 跳升至 16.2。同时 $h(4) = 8.3$ 是最大的单场景熵增——因为同时有 aging 效应和新的 SUSPECTS 边。
3. 到 $n = 5$，$H \approx 39$。如果我们设 $H_{\text{high}} = 35$，则张力压力 $P_\theta$ 已经激活释放模式，建议系统考虑解决一些悬而未决的问题。
...
- $P_\sigma(\sigma_1) \to 0$（秘密已公开，张力解除）
...
$H$ 从 $\approx 39$ 骤降至接近 $P_\rho$。这是一个典型的 Act III 特征：**主线压力的集中释放，伴随着新的、更深层的压力注入**——bob 的爱与 alice 的恨形成的情感张力成为新的叙事驱动力，为续篇埋下种子。
...
|------|---------|---------|-----------------|------|
| $n = 0$ | 3 | 4 | 中等 | 小型紧密图 |
| $n = 1$ | 6 | 11 | 增大 | 新节点紧密连接到既有网络 |
| $n = 4$ | 6 | 12 | 略增 | 新增 SUSPECTS 边增强连通 |
| $n = 6$ | 6 | 12 | 变化 | TRUSTS 被 HATES 替换，拓扑不变但权重变化 |
...
现代编剧理论提供了更具操作性的模型。Snyder[^2] 的三幕节拍结构为宏观叙事熵波形提供了经验基准（§6.3 定理 4）。Weiland[^6] 的角色弧线模型（Lie → Truth）可直接编码为 Character 粒子的 arc 属性。Swain[^1] 的 Scene-Sequel 模型——场景（目标-冲突-灾难）后接续场（反应-困境-决定）——启发了张力压力 $P_\theta$ 的交替自稳定设计。McKee[^7] 的冲突层级理论（内部冲突 → 人际冲突 → 外部冲突）对应了我们压力函数的多尺度结构。
...
**压力函数的参数化**。$P_\tau$ 中的 $\beta$，$P_\sigma$ 中的 $\alpha_k, \alpha_s$，$P_\theta$ 中的 $\delta_\pm, H_{\text{low}}, H_{\text{high}}$ 等参数在本文中是自由参数。它们的最优值可能因叙事类型（冒险 vs 日常 vs 悬疑）和受众偏好而异。建立参数选择的理论或经验指导是重要的未来工作。
...
2. **参数理论**：建立压力函数参数（$\beta, \alpha_k, \alpha_s, \delta_\pm$ 等）的选择理论——是否存在普适的最优参数，还是必须针对叙事类型和受众定制？
